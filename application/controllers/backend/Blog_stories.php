...
class Blog_stories extends CI_Controller {
	...
	public function index($id = ''){   
        ...
        if($this->input->get('get_list')){
		   ...
		   $postList   = $this->Stories->postList($rowperpage,$rowno,'post',$id, $search_string);
		   $total_posts  = $this->Stories->postListCount('post',$id, $search_string);
		   $data  = ['total_posts' => $total_posts ,'rowperpage' => $rowperpage,'postList' => $postList];
		   die(json_encode($data));
        }else{
        	$data['mainbodyContent'] = $this->load->view('backend/stories',$data,TRUE);
	...
	public function categories($id = ''){
		...
		if($this->input->get('get_list')){
        	$postData = json_decode($this->input->get('data'));
        	$search_string = $postData->search_string;
        	$postList = $this->Stories->postCaegoryList($id,$search_string);
        	die(json_encode($postList));
        }else{
        	$data['mainbodyContent'] = $this->load->view('backend/story_categories',$data,TRUE);
			$this->load->view('backend/master_view',$data);
        }
	}
	...
	public function add_categories(){
		...
		if((int)$id > 0 && $id != ''){
			$check = $this->Stories->slug_exists_category($slug, $id); 
			if($check == 0){
				$where = array('id'=>$id);
				$data  = array(
					'name'		 		=> $category_name,
				...
				$this->Stories->update_category($where, $data); 
				$jsonResponseData = [
									"status"   => "success",
                					"color"    => "green",
			...
			$check = $this->Stories->slug_exists_category($slug, ''); 
			if($check == 0){
				$data  = array(
						'name'		 		=> $category_name,
						'slug'		 		=> $slug,
				...
				$insert= $this->Stories->insert_category($data);
				$jsonResponseData = [
									"status"   => "success",
                					"color"    => "green",
	...
	public function add(){
		...
		if($this->form_validation->run() == true){  
			...
			$insert= $this->Stories->insert($data);
	...
	public function edit(){
        ...
        if(isset($_FILES['list_image']) && $_FILES['list_image']['name'] != ''){
			...
			$update= $this->Stories->update($where, $data); 
			if($update != ''){ 
	...
	public function get_slug(){
	    ...
	    return $this->Stories->slug_exists(trim($slug), trim($id) ,'page');
	}
