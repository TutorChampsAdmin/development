...
class Stories extends CI_Model{
	...
	public function get_page_details($slug , $type){
        ...
        $this->db->where('url_slug', trim($slug));
         $this->db->join('story_categories pc', 'pc.id = p.category_id', 'left');
   ...
   public function update($where, $data) {  
    ...
    public function insert_category($data = array()){
        ...
        $insert   = $this->db->insert('story_categories',$data);
        if($insert){
            return $this->db->insert_id();
        }else{
            return false;
   ...
   public function update_category($where, $data) {  
        $this->db->update('story_categories', $data, $where);
        return true;
    }
    ...
    public function slug_exists($slug, $id = '', $type){   
     ...
     public function all_post_lists($limit, $start, $catId = '') {
        ...
        $this->db->from('stories as q');
        $this->db->join('user u','u.id = q.created_by','left');
        $this->db->join('story_categories pc','pc.id = q.category_id','left');
   ...
   public function get_category_list($limit = '0') {
    ...
    public function post_next_link($currentPostId){
        $query = $this->db->query("SELECT title, url_slug FROM stories WHERE type = 'post' AND id = (SELECT MIN(id) FROM stories WHERE id > $currentPostId AND type = 'post')");
        return json_decode(json_encode($query->row()), True);
    }
   ...
   # Prevoius Post Link
    ...
    public function post_previous_link($currentPostId){
        $query = $this->db->query("SELECT title, url_slug FROM stories WHERE type = 'post' AND id = (SELECT MAX(id) FROM stories WHERE id < $currentPostId AND type = 'post')");
        return json_decode(json_encode($query->row()), True);
    }
  
  public function get_related_post($category, $currentPost){

     $query = $this->db->query("SELECT title, url_slug ,featured_image, created_date  FROM stories WHERE ( category_id IN ($category) ) AND id != $currentPost AND category_id != '0' AND latest_view_date  >= (CURDATE() - INTERVAL 90 DAY) ORDER BY page_view_count LIMIT 3");
     return $query->result_array();
  }
   ...
   public function count_visit($id){
    ...
    $this->db->query("UPDATE stories SET page_view_count = (page_view_count +1) , latest_view_date = '".$date."' WHERE id = ".$id."");
   }     
}